    ...
    const handleSave = () => {
        try {
            ...
            if (!Array.isArray(parsedQuestions) || parsedQuestions.some(q => !q.question || !Array.isArray(q.options) || typeof q.correctOption !== 'number' || typeof q.points !== 'number' || typeof q.duration !== 'number')) {
                Alert.alert('Invalid JSON', 'Please ensure the JSON array contains valid question objects with "question", "correctOption", "points", and "duration".');
                return;
            }
            const questionsWithIds = parsedQuestions.map(q => ({ ...q, id: q.id || uuid.v4() })); // Ensure IDs for new questions
            onSave(questionsWithIds);
            setJsonInput('');
        } catch (error) {
